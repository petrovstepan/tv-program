...
const sliceChannels = (channels, channelsOffset) => {
    return channels.slice(channelsOffset, +channelsOffset + numberOfChannelsToLoad)
}


const progFilter = (p) => progIsShowingNow(p.start, p.duration) || progWillRunInTheFuture(p.start)
...
const filterProgramsToCurrentTime = (programs, sliceForTvProgram = false) => {
    Object.keys(programs).forEach(xvid => {
        programs[xvid] = selectActiveProgram(
            sliceForTvProgram && sliceProgramsForTvProgram(programs[xvid].filter(progFilter)) ||
...
const sliceProgramsForTvProgram = (programs) => programs.slice(0, numberOfProgramsToShow)
const parseDateStringForSafari = (programs) => programs.map(p => ({...p, start: p.start.split('-').join('/')})) // костыль для сафари, дату через дефис не понимает
...
const combineChannelsAndPrograms = (channelArray, programs) => {
    const channels = {}
    channelArray.map(ch => {
        channels[ch.chid] = { ...ch, programs: programs[ch.xvid] && selectActiveProgram(programs[ch.xvid]) || [] }
    })
    return channels
}
